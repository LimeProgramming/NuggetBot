...
def req_ensure_env():
    ...
    try:
        ...
        assert os.path.isdir('nuggetbot'), 'folder "nuggetbot" not found'
        assert os.path.isfile('nuggetbot/__init__.py'), 'nuggetbot folder is not a Python module'

        assert importlib.util.find_spec('nuggetbot'), "nuggetbot module is not importable"
    except AssertionError as e:
        log.critical("Failed environment check, %s", e)
        bugger_off()

    try:
        os.mkdir('nuggetbot-test-folder')
    except Exception:
        ...
        rmtree('nuggetbot-test-folder', True)
...
def main():
    ...
    while tryagain:
        ...
        m = None
        try:
            from nuggetbot.bot import NuggetBot
            m = NuggetBot()
